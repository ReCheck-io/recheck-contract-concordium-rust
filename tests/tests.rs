...
#[test]
fn test_non_owner_cannot_create_record() {
    let (mut chain, init) = initialize();

    let bob_address = AccountAddress([1u8; 32]);
    let bob_account = Account::new(bob_address, ACC_INITIAL_BALANCE);
    let bob_signer = Signer::with_one_key();
    chain.create_account(bob_account);
    ...
    let create_record_params = CreateRecordParams {
        record_id: random_record_id.clone(),
        trail: random_trail.clone(),
        trail_signature: random_trail_signature.clone(),
    };

    let result = chain.contract_update(
        bob_signer,
        bob_address,
        Address::Account(bob_address),
        Energy::from(5000),
    ...
    assert!(result.is_err(), "Bob should not be allowed to create a record");

    if let Err(error) = result {
        if let Some(return_value) = error.return_value() {
            let error_message = String::from_utf8_lossy(&return_value);
            assert!(error_message.contains("Unauthorized"), "Expected an Unauthorized error");
        } else {
...
fn test_create_and_get_new_record() {
    ...
    assert_eq!(hex::encode(returned_record.record_id), random_record_id);
    assert_eq!(hex::encode(returned_record.parent_record_id), random_record_id);
    assert_eq!(hex::encode(returned_record.trail), random_trail);
    assert_eq!(hex::encode(returned_record.trail_signature), random_trail_signature);
...
fn test_create_and_get_new_sub_record() {
    ...
    let create_record_params = CreateRecordParams {
        record_id: random_parent_record_id.clone(),
        trail: random_parent_trail.clone(),
        trail_signature: random_parent_trail_signature.clone(),
    };
    ...
    let create_sub_record_params = CreateSubRecordParams {
        record_id: random_sub_record_id.clone(),
        parent_record_id: random_parent_record_id.clone(),
        trail: random_sub_trail.clone(),
        trail_signature: random_sub_trail_signature.clone(),
    };
    ...
    chain.contract_update(
        ...
        UpdateContractPayload {
            ...
            message: OwnedParameter::from_serial(&create_sub_record_params).expect("Serialize create sub record params."),
        },
    ...
    let view_list = chain.contract_invoke(
        ...
        UpdateContractPayload {
            ...
            message: OwnedParameter::from_serial(&SubRecordParams {
                sub_record_id: random_parent_record_id.clone(),
                index: 0,
    ...
    assert_eq!(hex::encode(returned_record.record_id), random_sub_record_id);
    assert_eq!(hex::encode(returned_record.parent_record_id), random_parent_record_id);
    assert_eq!(hex::encode(returned_record.trail), random_sub_trail);
    assert_eq!(hex::encode(returned_record.trail_signature), random_sub_trail_signature);
    assert_eq!(returned_record.creator, Address::Account(ALICE));
    assert_eq!(returned_record.sub_records_length, 0);
}
...
fn test_create_and_get_new_record_with_extras_2() {
    ...
    let create_sub_record_with_extras2_params = CreateSubRecordWithExtras2Params {
        record_id: random_record_id.clone(),
        parent_record_id: random_record_id.clone(),
        trail: random_trail.clone(),
        trail_signature: random_trail_signature.clone(),
        extra0: random_extra_0.clone(),
        extra1: random_extra_1.clone(),
    };
    ...
    chain.contract_update(
        ...
        UpdateContractPayload {
            ...
            message: OwnedParameter::from_serial(&create_sub_record_with_extras2_params).expect("Serialize create sub record with extras2 params."),
        },
...
fn test_create_non_unique_record() {
    ...
    let create_record_params_2 = CreateRecordParams {
        record_id: random_record_id.clone(),
        trail: random_trail_2.clone(),
        trail_signature: random_trail_signature_2.clone(),
    };
...
fn test_get_non_existing_record() {
    ...
    assert_eq!(hex::encode(returned_record.record_id), hex::encode([0u8; 32]));
}
